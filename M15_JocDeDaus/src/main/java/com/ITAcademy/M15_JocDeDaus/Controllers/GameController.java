...
public class GameController {
	...
	public ResponseEntity<Message> deleteGames(@PathVariable String player_id) {
		...
		if (playerGames.isEmpty()) {
			return new ResponseEntity<Message>(
					new Message("Player " + playerName + " with id " + player_id + " has not played any game yet.", null, ""), HttpStatus.OK);
		}
		gameService.deleteGamesByPlayer(player_id);
		return new ResponseEntity<Message>(new Message("Deleted all games of player: " + playerName + ", with id " + player_id, "", ""),
				HttpStatus.OK);
	}
}			